...
import { getContactEnquiries, updateContactEnquiry } from '../../lib/api/contacts'
...
export default function DashboardEnquiriesPage() {
  const { data, loading, error, run: fetchEnquiries } = useApi(getContactEnquiries)
  const { run: updateEnquiry } = useApi(updateContactEnquiry)
  ...
  const handleAssign = async (id, assigned_to) => {
    setUpdatingId(id)
    try {
      await updateEnquiry(id, { assigned_to: assigned_to || null })
      runQuery()
    } finally {
      setUpdatingId(null)
    }
  ...
  const handleNotesSave = async (id) => {
    ...
    setUpdatingId(id)
    try {
      await updateEnquiry(id, { internal_notes })
      runQuery()
    } finally {
      setUpdatingId(null)
    }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (search) params.search = search
      if (statusFilter) params.status = statusFilter
      const full = await getContactEnquiries(params)
      ...
      exportToCsv(
        ...
        `contact-enquiries-${new Date().toISOString().slice(0, 10)}.csv`
      )
    } finally {
      setExporting(false)
    }
    ...
    <>
      ...
      <DashboardLayout title="Contact Enquiries">
        ...
        {!loading && enquiries.length === 0 && (
          <p className="text-sm text-[var(--muted)]">No enquiries found.</p>
        )}
        ...
        <div className="space-y-3">
          {enquiries.map((enquiry) => (
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 gap-3 mt-4 pt-4 border-t border-white/5">
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <textarea
                      ...
                      value={notesDraft[enquiry.id] ?? enquiry.internal_notes ?? ''}
                      onChange={(e) =>
                        setNotesDraft((prev) => ({ ...prev, [enquiry.id]: e.target.value }))
                      }
